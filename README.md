...
5. `gen_NMF()` wraps: Variational Bayes Estimation by calling `vb_estimate()`, generates and stores all of the Negative Factorized Matricies by calling `algo1_only_store_draws()`, which uses `find_NMF_given_solution()` to factorize.  (around 150sGB)
